...
const CourseDiscussion = () => {
  ...
  const colors = {
    ...
    amberLight: "#fff8e1",
    dark: "#2d2d2d",
    verifiedBg: "#e6f4ea" // Light green for verified answers
  };
  ...
  const submitAnswer = async () => {
    ...
    setAnswerText("");
    openQuestionDetail(selectedQuestion);
  };

  const upvote = async (id) => {
    await api.put(`/forum/answer/upvote/${id}`);
    openQuestionDetail(selectedQuestion);
  };
                  ...
                  <div className="d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-2">
                    <div className="flex-grow-1">
                      <div className="d-flex align-items-center mb-1">
                        <h6 className="fw-bold mb-0 text-start" style={{ color: colors.dark }}>{q.title}</h6>
                        {q.isSolved && <CheckCircle size={18} className="text-success ms-2" />}
                      </div>
                      <p className="small mb-0 text-muted text-start">
                        <User size={14} className="me-1" /> Posted by <span className="fw-bold" style={{ color: colors.purple }}>{q.userId?.name || 'User'}</span>
                      </p>
                    </div>
          ...
          <div className="modal-dialog modal-dialog-centered modal-lg px-3">
            <div className="modal-content border-0 shadow-lg rounded-4">
              ...
              <div className="modal-footer border-0 pb-4 px-4">
                <button className="btn btn-link text-decoration-none text-muted fw-bold" onClick={() => setShowAskModal(false)}>Cancel</button>
                <button className="btn px-5 rounded-pill shadow fw-bold" 
                  style={{ backgroundColor: colors.amber, color: colors.dark }}
            ...
            <div className="modal-content border-0 shadow-lg rounded-4" style={{ maxHeight: '92vh' }}>
              ...
              <div className="modal-body p-4" style={{ backgroundColor: '#fafafa' }}>
                <div className="bg-white p-4 rounded-4 shadow-sm mb-4 border-bottom border-4" style={{ borderBottomColor: colors.amber }}>
                  <p className="text-dark mb-0 lh-lg" style={{ whiteSpace: 'pre-line' }}>{selectedQuestion.description}</p>
                </div>

                <div className="d-flex align-items-center mb-3">
                  <h6 className="fw-bold m-0" style={{ color: colors.purple }}>Responses 📩</h6>
                  <span className="ms-2 badge rounded-pill px-3" style={{ backgroundColor: colors.purpleLight, color: colors.purple }}>{answers.length}</span>
                </div>

                <div className="d-flex flex-column gap-3">
                  {answers.length === 0 ? (
                    <div className="text-center py-4 rounded-4" style={{ border: `2px dashed ${colors.purple}`, backgroundColor: '#fff' }}>
                      <p className="small mb-0 opacity-50">Be the first to share an answer! ✍️</p>
                    </div>
                  ) : (
                    answers.map((ans) => (
                      <div 
