...
export default function Profile() {
  ...
  // --- NAYA: Skill Proficiency Visualizer ---
  const renderSkillProficiency = (skills) => {
    if (!Array.isArray(skills) || skills.length === 0) return null;

    return (
      <div className="mt-5">
        <h5 className="fw-bold mb-4 d-flex align-items-center gap-2" style={{ color: "#4C1D95" }}>
          <FiBarChart2 size={22} /> Skill Proficiency Intelligence
        </h5>
        <div className="row g-4">
          {skills.map((s, idx) => {
            const level = s.level || 0;
            let color = "#F98080"; // Default Red
            let status = "Learning";
            let bg = "#FDF2F2";
        ...
        {xpByCourse.map((item, idx) => (
          <div className="col-md-6" key={idx}>
            <div className="d-flex align-items-center justify-content-between p-3 rounded-4 bg-white border border-light shadow-sm">
              ...
              <div className="fw-bold" style={{ color: "#7C3AED" }}>{item?.xp ?? 0} <span className="small text-muted fw-normal">XP</span></div>
            </div>
          </div>
        ))}
      </div>
